...
const defaultAccessOpenedTemplate = `✅ LAYANAN AKTIF KEMBALI
...
const defaultMaintenanceTemplate = `🔧 PEMBERITAHUAN MAINTENANCE

Yth. Bapak/Ibu {nama},

Kami informasikan bahwa akan dilakukan maintenance/perbaikan pada jaringan kami.

📅 Waktu Mulai: {tanggal_mulai}
...
const notificationForm = useForm({
    ...
    access_opened_template: props.settings.access_opened_template || defaultAccessOpenedTemplate,
    maintenance_template: props.settings.maintenance_template || defaultMaintenanceTemplate,
})
...
const tripayForm = useForm({
    tripay_enabled: props.tripayConfig?.enabled || false,
    tripay_sandbox: props.tripayConfig?.sandbox ?? true,
    tripay_api_key: props.tripayConfig?.api_key || '',
    tripay_private_key: props.tripayConfig?.private_key || '',
...
const tabs = [
    ...
    { id: 'tripay', label: 'Payment Gateway', icon: 'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z' },
]
</script>
            ...
            <div class="flex-1">
                ...
                <div v-if="activeTab === 'notification'" class="bg-white rounded-xl shadow-sm p-6">
                    ...
                    <form @submit.prevent="saveNotification" class="space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Template Layanan Aktif Kembali</label>
                            <textarea v-model="notificationForm.access_opened_template" rows="10" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 font-mono text-sm"></textarea>
                            <p class="text-xs text-gray-500 mt-1">Variabel: <code class="bg-gray-100 px-1 rounded">{nama}</code> <code class="bg-gray-100 px-1 rounded">{customer_id}</code> <code class="bg-gray-100 px-1 rounded">{paket}</code> <code class="bg-gray-100 px-1 rounded">{telepon}</code> <code class="bg-gray-100 px-1 rounded">{whatsapp}</code></p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Template Maintenance / Gangguan</label>
                            <textarea v-model="notificationForm.maintenance_template" rows="12" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 font-mono text-sm"></textarea>
                            <p class="text-xs text-gray-500 mt-1">Variabel: <code class="bg-gray-100 px-1 rounded">{nama}</code> <code class="bg-gray-100 px-1 rounded">{tanggal_mulai}</code> <code class="bg-gray-100 px-1 rounded">{tanggal_selesai}</code> <code class="bg-gray-100 px-1 rounded">{keterangan}</code> <code class="bg-gray-100 px-1 rounded">{telepon}</code> <code class="bg-gray-100 px-1 rounded">{whatsapp}</code></p>
                            <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 mt-2 text-sm text-amber-800">
                                Template ini digunakan untuk mengirim notifikasi ke pelanggan saat ada jadwal maintenance server atau gangguan jaringan.
                            </div>
                        </div>
