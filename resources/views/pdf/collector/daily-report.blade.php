...
<head>
    <meta charset="UTF-8">
    <title>Laporan Harian Penagih</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="report-title">LAPORAN HARIAN PENAGIH</div>
        </div>
        ...
        <div class="info-section">
            <table style="width: 60%; border: none;">
                <tr style="border: none;">
                    <td style="border: none; padding: 3px 0;"><strong>Nama Penagih</strong></td>
                    <td style="border: none; padding: 3px 0;">: {{ $collector->name }}</td>
                </tr>
                <tr style="border: none;">
                    <td style="border: none; padding: 3px 0;"><strong>Tanggal</strong></td>
                    <td style="border: none; padding: 3px 0;">: {{ $date->format('d F Y') }}</td>
                </tr>
                <tr style="border: none;">
        ...
        <table>
            ...
            <tfoot>
                <tr>
                    <th colspan="5" class="text-right">Total Tunai:</th>
                    <th class="text-right">Rp {{ number_format($totals['payment_cash'], 0, ',', '.') }}</th>
                </tr>
                <tr>
                    <th colspan="5" class="text-right">Total Transfer:</th>
                    <th class="text-right">Rp {{ number_format($totals['payment_transfer'], 0, ',', '.') }}</th>
                </tr>
                <tr>
        ...
        <!-- Daftar Pengeluaran -->
        <div class="section-title">PENGELUARAN / BELANJA</div>
        @if($expenses->count() > 0)
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <span class="badge {{ $expense->status === 'approved' ? 'badge-success' : ($expense->status === 'rejected' ? 'badge-danger' : 'badge-warning') }}">
                            {{ $expense->status === 'approved' ? 'OK' : ($expense->status === 'rejected' ? 'Ditolak' : 'Pending') }}
                        </span>
                    </td>
        ...
        <div class="summary-box">
            <h3 style="margin-bottom: 10px; font-size: 12px;">RINGKASAN SETORAN</h3>
            <div class="summary-row">
                <span>Total Tagihan Tunai</span>
                <span>Rp {{ number_format($settlement['cash_collection'], 0, ',', '.') }}</span>
            </div>
            <div class="summary-row">
                <span>Total Pengeluaran (Approved)</span>
                <span>- Rp {{ number_format($settlement['approved_expense'], 0, ',', '.') }}</span>
            </div>
            @if($settlement['commission_amount'] > 0)
            <div class="summary-row">
                <span>Komisi ({{ $settlement['commission_rate'] }}%)</span>
                <span>- Rp {{ number_format($settlement['commission_amount'], 0, ',', '.') }}</span>
            </div>
            @endif
            <div class="summary-row">
                <span>SALDO YANG HARUS DISETOR</span>
                <span>Rp {{ number_format($settlement['must_settle'], 0, ',', '.') }}</span>
            </div>
        </div>
