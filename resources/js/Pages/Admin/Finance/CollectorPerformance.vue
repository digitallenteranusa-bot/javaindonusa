...
const applyFilter = () => {
    router.get('/admin/finance/collector-performance', {
...
const exportAll = () => {
    window.location.href = `/admin/finance/collector-performance/export?year=${selectedYear.value}&month=${selectedMonth.value}`
}

const exportCollector = (collectorId) => {
    window.location.href = `/admin/finance/collector-performance/export/${collectorId}?year=${selectedYear.value}&month=${selectedMonth.value}`
}
</script>
    ...
    <AdminLayout>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
            ...
            <div class="bg-white rounded-xl shadow-sm p-4">
                ...
                <p class="text-xl font-bold mt-1 text-emerald-600">
                    {{ formatCurrency(summary?.total_cash) }}
                </p>
            </div>
            <div class="bg-white rounded-xl shadow-sm p-4">
                <p class="text-sm text-gray-500">Total Transfer</p>
                <p class="text-xl font-bold mt-1 text-indigo-600">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="(collector, index) in collectors" :key="collector.id" class="hover:bg-gray-50">
                            ...
                            >
                                {{ formatCurrency(collector.net_income) }}
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="flex-1 bg-gray-100 rounded-full h-2 min-w-[60px]">
                                        <div
                                            ...
                                            :style="{ width: `${Math.min(collector.collection_rate, 100)}%` }"
                                        ></div>
                                    </div>
                            ...
                            <td class="px-4 py-4 text-center">
                                <button
                                    @click="exportCollector(collector.id)"
                                    class="px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg text-xs hover:bg-emerald-100 font-medium"
                                    title="Export detail penagih"
                                >
                                    Export
                                </button>
                            </td>
