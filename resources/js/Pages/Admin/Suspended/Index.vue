...
const unsuspendLoading = ref(false)

const openUnsuspendModal = (customer) => {
...
const confirmUnsuspend = () => {
    ...
    unsuspendLoading.value = true
    router.post(`/admin/suspended/${selectedCustomer.value.id}/unsuspend`, {}, {
        onFinish: () => {
            unsuspendLoading.value = false
            closeUnsuspendModal()
        },
    })
}
...
<template>
    <Head title="Pelanggan Cuti" />

    <AdminLayout>
        <template #header>
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Pelanggan Cuti</h1>
                <p class="text-sm text-gray-500 mt-1">Total: {{ customers.total }} pelanggan cuti sementara</p>
            </div>
        </template>
        ...
        <div v-else class="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Alasan Cuti</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Mulai Cuti</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Perkiraan Selesai</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="customer in customers.data" :key="customer.id" class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">
                                {{ formatDate(customer.suspension_start_date) }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">
                                {{ formatDate(customer.suspension_end_date) }}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <button
            ...
            <div v-if="!customers.data?.length" class="text-center py-12">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">Tidak ada pelanggan cuti</h3>
        ...
        <div v-if="showUnsuspendModal" class="fixed inset-0 z-50 flex items-center justify-center">
            <div class="fixed inset-0 bg-black bg-opacity-50" @click="closeUnsuspendModal"></div>
            <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md relative z-10">
                <h3 class="text-lg font-semibold text-gray-900 mb-2">Aktifkan Kembali</h3>
                <p class="text-sm text-gray-600 mb-1">
                    Apakah Anda yakin ingin mengaktifkan kembali pelanggan:
                </p>
                <div class="bg-gray-50 rounded-lg p-3 mb-4">
                    ...
                    <p v-if="selectedCustomer?.suspension_reason" class="text-sm text-yellow-600 mt-1">
                        Alasan cuti: {{ selectedCustomer.suspension_reason }}
                    </p>
                </div>
                <div class="flex justify-end gap-3">
                    <button
                        @click="closeUnsuspendModal"
                    ...
                    <button
                        ...
                        :disabled="unsuspendLoading"
                    >
                        {{ unsuspendLoading ? 'Memproses...' : 'Ya, Aktifkan' }}
                    </button>
                </div>
            </div>
        </div>
